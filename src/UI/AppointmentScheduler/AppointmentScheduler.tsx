    ...
    const commitChanges = ({ added, changed, deleted }) => {
        ...
        setState((prevState => {
            return {
                ... prevState,
                if (added) {
                    const startingAddedId = prevState.data.length > 0 ? prevState.data[prevState.data.length - 1].id + 1 : 0;
                    prevState.data = [...prevState.data, { id: startingAddedId, ...added }];
                },
                if (changed) {
                    prevState.data = prevState.data.map(appointment => (
                        changed[appointment.id] ? { ...appointment, ...changed[appointment.id] } : appointment));
                },
                if (deleted !== undefined) {
                prevState.data = prevState.data.filter(appointment => appointment.id !== deleted);
            }
